...
def validar(event):
    print('Se llamo metodo validar')
    usuario = usuario_caja_texto.get()
    password = password_caja_texto.get()
    if usuario == 'root' and password == 'admin':
        print('Usuario valido')
        showinfo(title='Login', message='Datos validados!!')
    else:
        print('Usuario invalido')
        showerror(title='Login', message='Usuario invalido')

# Asociar eventos al boton de login
login_boton.bind('<Return>', validar)
login_boton.bind('<Button-1>', validar)
